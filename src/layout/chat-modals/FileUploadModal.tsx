...
const FileUploadModal = ({ onCancel, onUpload }: { onCancel: () => void; onUpload?: (file: File) => void }) => {
  const { theme } = useTheme()
  const [img, setImg] = useState<File | undefined>()

  const handleClick = async () => {
    if (onUpload && img) onUpload(img)
  }
    ...
    <Modal className='bg-slate-100 opacity-90 rounded-lg lg:w-[70vh] xs:w-[40vh]  dark:bg-modalDarkColor'>
      ...
      <div className='flex flex-col '>
        ...
        <div className='flex gap-2 items-center justify-center mt-5 lg:w-[67vh] xs:w-[37vh]'>
          <Button type='button' variant='outlined' color='primary' children='Cancel' onClick={() => onCancel()} centerClassName='flex justify-center items-center' buttonClassName='!px-3 font-poppins py-3 w-full' />
          <Button
            // loading={isLoading}
            onClick={() => handleClick()}
            type='submit'
