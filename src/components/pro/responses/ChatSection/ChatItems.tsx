...
function ChatItems() {
  ...
  const fetchData = async () => {
    ...
    if (getChatDocument.docs.length > 0) {
      const getMessagesQuery = query(collection(db, "chats", getChatDocument.docs[0].id, 'messages'), orderBy("timestamp"));
      await onSnapshot(getMessagesQuery, async (querySnapshot) => {
        await setChats(await querySnapshot.docs?.map((doc) => {
  ...
  const handleSendMessage = async () => {
    const getChatQuery = query(collection(db, "chats"), where("chat_id", "==", combinedId));
    const getChatDocument = await getDocs(getChatQuery);
    await addDoc(collection(db, "chats", getChatDocument.docs[0].id, "messages"), {
      message: userInput,
    ...
    <div>
      <HomeCard className="rounded-md px-5 pb-5 lg:h-[85vh] ">
        ...
        <div className="bg-gray-100 p-4 rounded-lg shadow-md">
          ...
          <div ref={divRef} className="2xl:h-[63vh] flex flex-col xl:h-[59vh] lg:h-[50vh] xs:h-[60vh] overflow-y-scroll">
