...
function HomeTopBar(props: { isSettingDisabled?: boolean }) {
  ...
  return (
    <div className={topbarClassName}>
      {!isLoggedIn ? (<><div className="my-1 xs:w-3/6 lg:w-max">
        ...
        <div className="md:flex items-center md:gap-5 xs:flex xs:gap-2">
        ...
        </div></>) : (<>{openMenu && <TopBarMenu onClose={() => setOpenMenu(false)} />}
          ...
          <div className=" my-1 xs:w-3/6 lg:w-max">
            <button onClick={() => navigate("/home")}>
              <img src={ErranddoLogo} className="lg:w-80 xs:w-full object-contain" />
            </button>
          </div>
          <div className=" md:gap-3  xs:gap-3 flex  ml-auto items-center">
            <Button
              ...
              onClick={() => { navigate('/pro/dashboard'); localStorage.setItem("role", "pro") }}
            />
